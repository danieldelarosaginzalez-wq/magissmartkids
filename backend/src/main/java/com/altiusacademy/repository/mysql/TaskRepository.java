...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    // List<Task> findByStudentAndSubject(User student,
    // com.altiusacademy.model.entity.Subject subject);
    // List<Task> findByStudentAndSubjectAndStatus(User student,
    // com.altiusacademy.model.entity.Subject subject, Task.TaskStatus status);
    // List<Task> findByStudentAndStatusOrderByDueDate(User student, Task.TaskStatus
    // status);
    // List<Task> findByStudentAndStatusOrderByGradedAtDesc(User student,
    ...
    Double getAverageProgressByTeacherAndSubjectAndGrade(@Param("teacherId") Long teacherId,
            @Param("subjectId") Long subjectId, @Param("grade") String grade);

    @Query("SELECT t FROM Task t WHERE t.teacher.id = :teacherId AND t.subject.id = :subjectId AND t.grade = :grade")
    List<Task> findByTeacherAndSubjectAndGrade(@Param("teacherId") Long teacherId, @Param("subjectId") Long subjectId,
    ...
    List<Task> findByTeacherIdAndSubjectIdAndGrade(@Param("teacherId") Long teacherId,
            @Param("subjectId") Long subjectId, @Param("grade") String grade);

    @Query("SELECT COUNT(t) FROM Task t WHERE t.taskTemplate.id = :templateId AND t.status IN :statuses")
    Long countByTaskTemplateIdAndStatusIn(@Param("templateId") Long templateId,
    ...
    // Buscar tareas por grado y profesor (para que los estudiantes vean las tareas
    // de su grado)
    ...
    // 🆕 Buscar tareas por lista de IDs de materias (para estudiantes)
    @Query("SELECT t FROM Task t WHERE t.subject.id IN :subjectIds ORDER BY t.createdAt DESC")
    List<Task> findBySubjectIdIn(@Param("subjectIds") List<Long> subjectIds);
}
...
public class SuperAdminController {
    ...
    // Constructor manual para reemplazar @RequiredArgsConstructor
    public SuperAdminController(UserService userService, InstitutionService institutionService) {
    ...
    public ResponseEntity<Map<String, Object>> getSuperAdminStats() {
        try {
            System.out.println("📊 [GET] /api/super-admin/stats - Obteniendo estadísticas del sistema");
            ...
            double growthPercentage = allUsers.size() > 0 ? Math.min(25.0, (double) activeUsers / allUsers.size() * 100)
                    : 0;
            ...
            stats.put("activeSessions", Math.max(1, (int) (activeUsers * 0.3))); // Estimate 30% of active users have
                                                                                 // sessions
    ...
    public ResponseEntity<Map<String, Object>> getAllInstitutions() {
        try {
            System.out.println("🏛️ [GET] /api/super-admin/institutions - Obteniendo todas las instituciones");
            ...
            response.put("institutions", institutions.stream().map(inst -> {
                ...
                long studentCount = institutionUsers.stream().filter(u -> "student".equals(u.getRole().getValue()))
                        .count();
                long teacherCount = institutionUsers.stream().filter(u -> "teacher".equals(u.getRole().getValue()))
                        .count();
                ...
                double avgGrade = studentCount > 0 ? Math.max(2.5, Math.min(5.0, 3.5 + (studentCount * 0.01))) : 0.0;
    ...
    public ResponseEntity<Map<String, Object>> getAllUsers() {
        try {
            System.out.println("👥 [GET] /api/super-admin/users - Obteniendo todos los usuarios");
    ...
    public ResponseEntity<Map<String, Object>> getSystemMetrics() {
        ...
        // Mock system metrics - in a real implementation, these would come from
        // monitoring tools
    ...
    public ResponseEntity<Map<String, Object>> searchUsers(
        ...
        List<User> paginatedUsers = startIndex < totalElements ? filteredUsers.subList(startIndex, endIndex)
                : new ArrayList<>();
    ...
    public ResponseEntity<Map<String, Object>> searchInstitutions(
        ...
        List<Institution> paginatedInstitutions = startIndex < totalElements
                ? filteredInstitutions.subList(startIndex, endIndex)
                : new ArrayList<>();
        ...
        response.put("institutions", paginatedInstitutions.stream().map(inst -> {
            ...
            // Get users for this institution
            List<User> institutionUsers = userService.findUsersByInstitution(inst.getId());
            instData.put("students",
                    institutionUsers.stream().filter(u -> "student".equals(u.getRole().getValue())).count());
            instData.put("teachers",
                    institutionUsers.stream().filter(u -> "teacher".equals(u.getRole().getValue())).count());
            instData.put("coordinators",
                    institutionUsers.stream().filter(u -> "coordinator".equals(u.getRole().getValue())).count());
    ...
    public ResponseEntity<Map<String, Object>> getAuditLogs(
        ...
        // Mock audit logs for demonstration - in a real implementation, this would come
        // from a database
    ...
    public ResponseEntity<Map<String, Object>> getSystemConfig() {
        // Mock system configuration - in a real implementation, this would come from a
        // database or config service
    ...
    public ResponseEntity<Map<String, Object>> updateSystemConfig(@RequestBody Map<String, Object> updates) {
        // In a real implementation, this would validate and persist the configuration
        // changes
        Map<String, Object> response = new HashMap<>();
        response.put("message", "System configuration updated successfully");
        response.put("updatedFields", updates.keySet());
        response.put("requiresRestart",
                updates.containsKey("maintenanceMode") || updates.containsKey("maxUsersPerInstitution"));
